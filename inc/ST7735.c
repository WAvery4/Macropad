...
//------------ST7735_DrawBigPixel------------
// Color 4 pixels pixel around the given coordinates with the given color.
// Input: x     horizontal position of the pixel, columns from the left edge
//               must be less than 128
//               0 is on the left, 126 is near the right
//        y     vertical position of the pixel, rows from the top edge
...
//        color 16-bit color, which can be produced by ST7735_Color565()
// Output: none
void ST7735_DrawBigPixel(int32_t x, int32_t y, uint16_t color){
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[], uint16_t color){
	for (uint32_t i = 0; i < num; i++) {
		if (bufX[i] <= _maxX && bufX[i] >= _minX && bufY[i] <= _maxY && bufY[i] >= _minY) {
			int32_t xMap = (127 * (bufX[i] - _minX)) / (_maxX - _minX);
			int32_t yMap = ((127 * (_maxY - bufY[i])) / (_maxY - _minY)) + 32;
			ST7735_DrawBigPixel(xMap, yMap, color);
		}
	}
}
...
// Inputs: y is the y coordinate of the bar plotted
// Outputs: none
void ST7735_Bar(int32_t x, int32_t y, uint16_t color){
	if (x <= _maxX && x >= _minX && y <= _maxY && y >= _minY) {
		int32_t xMap = (127 * (x - _minX)) / (_maxX - _minX);
		int32_t yMap = ((127 * (_maxY - y)) / (_maxY - _minY)) + 32;
		ST7735_DrawFastVLine(xMap, yMap, 159-yMap, color);
	}
}
